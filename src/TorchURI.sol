...
library TorchURI {
    ...
    function _getScript() internal pure returns (string memory) {
        return HTML.script(string.concat(
            ...
            'document.addEventListener("DOMContentLoaded", function () {if (window.self !== window.top) {document.getElementById("connect-button").disabled = true; document.getElementById("pass-torch-button").disabled = true;} else {',
            'var connectButton = document.getElementById("connect-button");connectButton.addEventListener("click", async function() {'
            'const provider = await connectWallet();if (provider) {await ',
            mmProvider.ethereum_request(mmProvider.eth_accounts()),
            '.then(async function (accounts) { if (accounts.length === 0) { ethereum.enable().then(function (accounts) { setDotColor("green"); console.log(accounts);}).catch(console.error); } else { setDotColor("green"); console.log(accounts); } await checkAndChangeChain(provider, "0x1"); }).catch(console.error);}});}});',
    ...
    function _getBody(address _nftLocation) internal view returns (string memory) {
        return HTML.body(string.concat(
            ...
            '<div class="column"><div class="box box-inner" id="svg-box"><p>This contract : ', LibString.toHexStringChecksummed(address(this)), '</p><p>NFT contract : ', LibString.toHexStringChecksummed(_nftLocation), '</p><p>ChainId : ', LibString.toString(block.chainid), '</>',

            '<!-- [SVG stuff here] --></div></div></div>',
            _getScript()
        ));
    }

    function renderHTML(address _nft) internal view returns (string memory) {
